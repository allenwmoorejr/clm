...
export default function WatchPage() {
  ...
  useEffect(()=>{ fetch("/api/live").then(r=>r.json()).then(setData).catch(()=>setData({live:false})); },[]);
  ...
  return (
    <section className="container py-16">
      ...
      <p className="text-white/70 mt-2">{data?.live ? "Thanks for worshiping with us!" : "We’re not live right now — enjoy the latest message below."}</p>
            ...
            <>
              ...
              <div className="absolute inset-0 grid place-items-center">
                <div className="flex flex-col items-center gap-3">
                  <a
                    href={videoId ? `https://www.youtube.com/watch?v=${videoId}` : `https://www.youtube.com/channel/${process.env.NEXT_PUBLIC_CHANNEL_ID ?? ""}/live`}
                    target="_blank" rel="noreferrer"
                    className="btn-primary"
                  >
                    Watch on YouTube
                  </a>
                  {videoId && <button onClick={()=>setShowIframe(true)} className="btn-ghost">Play in Page</button>}
                </div>
              </div>
            </>
          ) : (
